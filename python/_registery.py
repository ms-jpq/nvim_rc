from .editor import auto_comp, auto_save, commentary, linter, lsp, prettier
from .editor import search as e_search
from .editor import surround, syntax, whitespace
from .modes import command, normal, poly, terminal, visual
from .operators import move, replace, search, sort
from .text_objects import entire, indent, line, paragraph, word
from .version_control import git
from .workspace import input, misc, navigation, theme, wm

____ = None
